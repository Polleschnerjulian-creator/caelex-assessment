...
- `diff-summarizer.server.test.ts` (11 tests): Anthropic-mocked
  amendment-detection coverage. Verifies COSMETIC_ONLY shortcut,
  HTML/script/comment stripping, malformed-JSON handling, keyChanges
  filtering. Class-faking Anthropic SDK via vi.hoisted.
- `chat-title-generator.server.test.ts` (15 tests): AI-generated chat
  title coverage. Quote/period stripping, 60-char cap, array-shaped
  content handling, 800-char prompt truncation, fire-and-forget on
  DB / API failures. Mocks prisma + buildAnthropicClient + logger.
- `mandate-scaffold-context.server.test.ts` (10 tests): shared
  scaffold-context loader covering null mandateId early-returns,
  access-gated query scoping, parties orderBy spec, owner-null
  normalisation.

Total atlas test count: 412 passing (1 pre-existing auto-embed env
failure unchanged).

**2026-05-26**: T0.3 audit-log hash-chain coverage —
...
- 🟡 **T0.3** Test-Coverage on critical paths — SUBSTANTIALLY ADVANCED (174 new tests across 9 files, 2026-05-26). All 11 tool-bundle/runner files covered + 4 critical helpers retrofitted (audit-log, citation-extractor, diff-summarizer, chat-title-generator, mandate-scaffold-context). atlas-encryption was already covered. **Atlas server-side test count: 412 passing / 413 total** (1 pre-existing environmental failure: auto-embed, needs OpenAI API key). Remaining untested: chat-engine (1888 LOC — integration-test territory), document-tools (893 LOC — heavy Prisma/file mocks needed), document-processor (732 LOC — file processing), semantic-corpus (222 LOC — dynamic-import + fs mocks).
- 🟡 **T0.4** Audit-Log silent-failure elimination — PARTIAL (2026-05-26). `appendAtlasAudit` now retries up to 3× with exponential-style backoff (100ms / 500ms / 2s) before falling through to error-log. Hash chain remains valid under retry (each attempt re-fetches prevHash so a concurrent writer's insert produces a valid chain). Warn-logs on retry-success surface DB-flakiness to ops. 5 new tests + 1 existing test updated for retry semantics. Remaining: persistent `FailedAuditLog` table for retry-exhausted writes (needs Prisma migration); `/api/atlas/admin/audit-health` endpoint.
- 🔴 **T0.5** Encryption-Migration finalization (dual-read → encrypted-only)
- 🟡 **T0.6** `atlas-tools.ts` schema cleanup — SIDECAR APPROACH (2026-05-26). Instead of refactoring 48 Anthropic.Tool entries (risk: SDK rejects unknown fields), added parallel `tool-metadata.ts` sidecar with `bundle` / `requiresApproval` / `expectedDurationMs` / `costClass` per tool. Drift-safety test enforces `ATLAS_TOOLS ↔ TOOL_METADATA` match — fails CI if a developer adds a tool without registering metadata. Unlocks future UI hints (cost-class badges, duration progress bars, approval-gate modals) + audit-log enrichment + Pipeline-runner approval-gates (T1.E.26). 15 unit tests including drift-detection. Remaining: wire metadata into audit-log enrichment + UI consumers.
...
- 🟢 **T1.B.12** Real-time Source-Polling Cron — fully wired end-to-end: `/api/cron/atlas-source-check` (433 LOC) polls sources, computes content-hash diffs, calls `summariseDiff` (Anthropic Haiku) to classify cosmetic vs substantive, persists to `AtlasSourceCheck` (admin queue) + `AtlasSourceCheckHistory`. On admin approval via `/api/admin/atlas-updates`, `dispatchSourceAmendment` (notify.ts) fans out `AtlasNotification` rows per subscriber (subscribers union: SOURCE + JURISDICTION, deduped on userId). The track_amendment tool (T1.B.11) writes the AtlasAlertSubscription rows the fan-out reads. Coverage: notify.ts now has 12 unit tests (2026-05-26) — fan-out invariants, dedup, non-blocking failure mode.
...
- 🟡 **T1.E.25** Pipeline-Step UI (progress component) — HTTP route shipped 2026-05-26: `POST /api/atlas/workflows/[id]/run` bridges the pipeline runtime to clients. Auth-gated via getAtlasAuth, rate-limited (`assessment` tier), validates body via Zod (bypassApproval / mandateId / language / abortOnEmptyTurn / toolToggles / retryPolicy). Returns 404 for unknown workflows, 400 for non-pipeline workflows, 200 with `awaitingApproval` for halted pipelines, 200 with full result on completion, 500 for runner crashes. maxDuration=300s. 13 unit tests cover auth/rate-limit/validation/abort-code-mapping/payload-forwarding/error-handling. Remaining: React component for live progress + approval modal UI.
