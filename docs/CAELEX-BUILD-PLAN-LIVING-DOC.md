...
**Sprint 7 — Live-Streaming-Backbone** ✅ COMPLETED 2026-05-02
...
- Sprint 7E: Live-Astra-Reasoning-Stream UI (Wow-Pattern #5) ✅ COMPLETED 2026-05-02
...
- Sprint 8C: Public-Verify-Page (regulator pastes hash + proof, gets verified) ✅ COMPLETED 2026-05-02
- Sprint 8D: C2SP tlog-witness endpoint (Caelex acts as a witness) ⏸ DEFERRED — needs Ed25519 key infrastructure + signed-note format parsing + Merkle-tree consistency-proof verification (sumdb-style). Not session-scoped; revisit when a partner log requests Caelex as a witness.
...
- Sprint 12B: Top bar + breadcrumbs + cmd-K palette trigger to Glass-Regular ✅ COMPLETED 2026-05-04
...
8. Sprint 12B — Caelex Liquid Glass Top Bar (`V2TopBar.tsx` 48px sticky Glass-Regular edge-to-edge with breadcrumbs left + ⌘K pill center + bell+avatar right; `breadcrumbs.ts` pure helper turns pathname into Breadcrumb[] with static-label table mirroring V2Sidebar nav (Time Travel, Audit Center, Article Tracker, etc.) plus title-case fallback for unknown segments and id-truncation heuristic for cuid/uuid-shaped segments; ChevronRight separators; last crumb gets aria-current="page"; ⌘K pill is 320px-wide pill button reusing the canonical CommandPalette window-keydown event so no new state wiring; bell button is placeholder for Sprint 12F+ notification work; avatar pill renders userName+email with initial-square; V2Shell layout restructured to nest top-bar above main inside a flex content column, sidebar unchanged; cmd-K trigger removed from V2Sidebar — sidebar tightens to brand+nav+user-footer only; 9 V2TopBar tests + 12 breadcrumbs tests + 12 V2Sidebar regression — all 33 green)

### Previous batch (deployed `6d2fd6a3` on 2026-05-02)

1. Sprint 7D — Mission Ops Console UI (Wow-Pattern #3)
2. Sprint 7E — Live Astra Reasoning Stream (Wow-Pattern #5)
3. Sprint 8A — OpenTimestamps audit-anchor cron
4. Sprint 8B — OpenTimestamps audit-anchor upgrade-cron
5. Sprint 8C — Public verify page (no Caelex trust required)
6. Sprint 10A — Hash-Chain-Block-Visualizer (Wow-Pattern #7; pulled forward, 8D deferred)
