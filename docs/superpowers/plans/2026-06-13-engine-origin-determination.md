...
| W — Wrap (score reassessment + boards + PF-2 decision)                    | 🟡     | (see Score Reassessment §)                                          | **W1 ✅** score reassessment documented (below): safety axis 100/100, functional rubric 6/7, golden 124/346/274, all maturities lifted. **PF-2 ✅** decided (resolveOrderOfReview + 2 helpers dead → delete; spawned as a follow-up task, ListId stays). **W2 ⬜** AVA VerdictPanel licence-detail rendering — the last item; data already in `licenseDetermination`. AU/AUKUS conservative under-grant noted (safe, optional refinement).                                                                                                                                                                                                                                                                                                     |
