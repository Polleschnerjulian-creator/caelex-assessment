...
| B — Authz & API gate        | ✅ DONE        | see `git log` feat/fix(trade) | B1✅ getTradeAuth() · B2✅ **T-H1** all 28 routes product-gated (42 handlers, IDOR intact, +8 AI-route tests) · B3✅ **T-H10** AI screen_trade_party now read-only (no un-gated write/email; screenParty unreachable from AI path; engine.ts untouched=Comply-safe). 354 tests green. DEFERRED (own cross-surface task, touches shared Astra engine→affects Comply): engine-level ALL_TOOLS product-gating + auditor read-only enforcement = **B3-DEFER**. API 72→~90, Security 70→~88. |
