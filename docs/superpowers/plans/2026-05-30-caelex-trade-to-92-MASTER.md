...
| --------------------------- | -------------- | ----------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| A — Screening hardening     | 🟡 3 of 8 done | see `git log` feat/fix(trade) | A1✅ token-set helper · A2✅ scoreEntry max(jaro,token-set) **T-H2 closed** · A3✅ exact identifier (type+value) match **identifier-gap closed**. 299 screening tests green; all via implementer+review gates. NEXT: A4 fail-closed on missing snapshot (T-H3) → A5 missingLists all 8 → A6 control_no_equity → A7 OFAC aliases → A8 cross-screen dedup |
...
**Closes:** T-H7 (SAG race), T-H12 (Float money), T-M8 (@@unique), T-M9 (onDelete).
