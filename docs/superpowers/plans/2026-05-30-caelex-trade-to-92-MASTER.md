...
| D — Schema & money          | ✅ DONE (4/4) DEPLOYED                | fix/trade-to-92: 7732778b·c679278a·95050c41·c467ea4d·9f7411ae          | T-H7✅ atomic SAG draw-down (no overdraw) · T-M8✅ @@unique · T-M9✅ onDelete SetNull · **T-H12✅ Float→BigInt euro-cents COMPLETE** — verified prod DB had 0 money rows so the column flip is data-safe; new `lib/trade/money.ts` (toCents/fromCents, mirrors UK-ECJU pence); 6 schema fields→BigInt; all 21 conversion sites wired (service math in cents, euros↔cents at edges; T-H7 increment now toCents); tsc driven back to EXACTLY the 733 baseline (zero net new errors); 112 tests green. ES2017 target → BigInt() not 0n. db push applies the col flip during the Vercel build (verify post-build all 6 cols = bigint). **ALL Trade HIGH findings (T-H1…H12) now CLOSED.** Schema/Data-model →~92.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| H — Data corpus             | 🟡 part 1 DONE 2026-06-10             | feat/passage-genial (ILA batch)                                        | H-part-1 ✅: **EU Annex I Cat 1+2** — 54 verified supplier entries (composites/fibres + 1B production equipment, superalloy AM powders 1C002, ceramics, propellant constituents 1C111-family MT, nuclear-grade 1C2xx NP, bearings 2A001, machine tools 2B001/2B201, HIP/flow-forming/metrology/vibration, GTN software/tech shadows) wired into the normalized corpus + 7 tests. CW/BW lists deliberately headline-only and SAY SO (coverage.excluded). Remaining: Cat 0/8 depth as needed.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| ILA-Verbesserungen (Review) | ✅ DONE 2026-06-10                    | feat/passage-genial (8 commits)                                        | Founder "alles umsetzen" aus dem Passage-Review vor der ILA: #6/#10 hub trust signals (Listen-Frische-Strip + SAG-cap action items 80/95%) · #3 AVA-brand hero CTA · #1 [DEMO]-Workspace-Loader (echte Engines, entfernbar, MANAGER+) · #9 trade-digest cron 06:45 (OWNER/ADMIN, offene Posture, ehrlich, [DEMO] exkludiert) · #5 Org-Präzedenzfälle (Lern-Schleife ohne Schema-Change; Apply = USER_DECLARED+REQUIRES_REVIEW) · #8 CSV-Import (ERP-tolerant, Cap 200, draft-first) · #7 = H-part-1 + **G4-Eval-Harness** (scripts/trade-classification-eval.ts, EVAL_CONFIRM_COSTS-Guard, NICHT gelaufen; extractor modelOverride bypasses vision cache) · #2 public teaser **/passage/check** (corpus-only, zero AI, honesty-contract getestet). Sweep: 2089 green; 2 PRE-EXISTING z34c-Fails (bisect-bewiesen; Chip task_fee7a3a2). tsc 666 (0 neu).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| Post-ILA Hardening (Batch)  | ✅ DONE 2026-06-11                    | feat/passage-genial                                                    | Founder "mach den Rest, keine externen Kosten": **z34c-FN-Fix** (thrustNewtons T-M19 sanity cap 100kN→1e8N — EP-kalibrierte Range degradierte echte chemische Triebwerke zu UNKNOWN; 229 Matcher-Tests grün; Chip task_fee7a3a2 closed) · middleware.test 4 stale fails repariert + 3 NEUE security tests (HSTS-prod-Import, forged-cookie-bounce, MFA-challenge) · 7 weitere stale Atlas-Testfails repariert (attach-mandate cuid-Härtung M05/L3, auto-embed H15/SEC-T0-1-Drift; alle bisect-/worktree-bewiesen pre-existing) · swagger-ui-react entfernt · **Atlas Konflikt-Check KOMPLETT** (firm-wide API, detect-on-write, Banner+Clearance-Dialog+Index-Panel, 14 Gate-Tests; Tabelle via db push beim Deploy) · Upstash Redis AKTIV in prod (KV\_\* resolver). Atlas-Sweep 750/750. tsc 666 (0 neu). G4-Eval weiter user-gated (Kosten).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| Space-International S0–S6   | ✅ DONE 2026-06-13                    | feat/trade-space-international                                         | NEUE Kampagne (eigene Spec+Plan `2026-06-12-trade-*`). **S0–S4 DEPLOYED prod `6e87f3a4` (Ready, 34 Commits).** **S5–S6 = 15 Commits `d68fb3d2..a0885515` UNGEPUSHT (2 Sprints — Batch-Schwelle 6-8 NICHT erreicht, warten auf `deploy now`).** **S0** Origin-Routing Kreis A (EU-27/UK/US/CH/NO/CA/JP/AU/KR/IN) + fail-closed (Gate 4.5 + AVA) + **Gate 1.6 RU/BY-Destinationsverbot (PF-1)** + Golden-Set 744 Zellen. **S1–S6 Korpus:** Korpus +~880 verifizierte Einträge — MTCR 179·USML-IV 66·UK 123·EU-CML 25·CH/NO/CA/AU/KR-Mirror 196·Wassenaar 70→109·JP 97·IN 74. Mirror-Architektur (`adaptMirrorEntries`, no-dangling-Invariante). **JEDER Sprint adversariell quellen-verifiziert (W6); IN_SCOMET einmal REFUTED→gefixt→re-VERIFIED.** Origin-Maturity ALLER Nicht-EU/US-Regime BLEIBT 3 (UK-Lektion: dualUsePrimary ohne Engine-Origin-Determination = sonst false-CLEARED); Wassenaar→1. Golden byte-identisch 74/396/274 über ALLE Sprints (Korpus-Sprints = Präzision, nicht Verdicts). Konsolidiert 4699/4699, tsc 674. **W6 deckte + fixte 6 prä-existente BASIS-Mislabels (6A107·9.A.6·9A011·9.A.11·9A004.b·9A007·6A002·6A004) — alle amtlich gegenverifiziert.** OFFEN: S7 (Frische-Audit) · **systematischer Basis-Korpus-Audit** (Chip `task_ecf3aed1`) · Engine-Phase (Origin-Determination → hebt Maturities) · PF-2 (resolveOrderOfReview toter Code). **S1–S6 warten auf `deploy now`.** |
| Docs reconcile              | 🟡 CLAUDE.md done                     | feat/passage-genial                                                    | CLAUDE.md: ~40 Tiers statt 19 (ratelimit.ts = source of truth), /docs/api nicht mehr "Swagger UI", Upstash-Aktivierung notiert. REST offen: README/API-docs-Abgleich.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
