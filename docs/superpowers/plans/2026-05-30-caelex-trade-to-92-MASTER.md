...
| --------------------------- | -------------- | ----------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| A — Screening hardening     | 🟡 4 of 8 done | see `git log` feat/fix(trade) | A1✅ token-set · A2✅ scoreEntry max(jaro,token-set) **T-H2** · A3✅ identifier type+value match **id-gap** · A4✅ fail-closed on missing critical list **T-H3** (REVIEW_REQUIRED, no migration). 301 screening tests green; all review-gated. NEXT: A5 missingLists all 8 (T-M12) → A6 control_no_equity (T-H5) → A7 OFAC aliases (T-H4) → A8 cross-screen dedup (T-M2) |
