...
| --------------------------- | -------------- | ----------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| A — Screening hardening     | 🟡 5 of 8 done | see `git log` feat/fix(trade) | A1✅ token-set · A2✅ scoreEntry **T-H2** · A3✅ identifier type+value · A4✅ fail-closed **T-H3** · A5✅ missingLists all 8 **T-M12**. 303 screening tests green. NEXT: A6 control_no_equity (T-H5) → A7 OFAC aliases (T-H4) → A8 cross-screen dedup (T-M2). FOLLOW-UP for Sprint D: TradeSanctionsList enum lacks EU_ANNEX_IV + OPEN_SANCTIONS → euAnnexIv parser reuses BIS_DPL, openSanctions reuses OTHER (misleading missing-list labels; collision risk if real BIS_DPL parser added). Needs enum migration. |
...
**Closes:** §4 root cause #8 + the prod-readiness gap.
