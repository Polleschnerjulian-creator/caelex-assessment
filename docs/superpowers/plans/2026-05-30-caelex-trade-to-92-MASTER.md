...
| C — Correctness bugs        | 🟡 3 of 6, NOT deployed | branch `fix/trade-to-92` +6, commits 37b65f1d·f868fe04·a7f1d0b4·9a582bad | C1✅ **T-H6** item-classify no longer treats countryOfOrigin as destination · C3✅ **T-M5** Annex-IV Art.2b gate reads actual ECCN · C6✅ **T-M16** USML XV(e)(11)(iv) EP predicate fixed (real thrust/Isp/power attrs). 1051 comply-v2/trade tests green (objective exit=0). ⚠️ COMMITTED, NOT YET REVIEW-GATED OR DEPLOYED — paused mid-sprint (see glitch note). REMAINING: **C2** (T-M3/M4: legacy de-minimis-calculator has Sudan in E:1 [CONFIRMED real at HEAD: "SD"=1] + D:1=10% [should be 25%; THRESHOLD_RESTRICTED=10 confirmed] → reconcile to authoritative cascade.ts) · C4 (T-M19 unit-awareness) · C5 (T-M20 order-of-review same-regime drop). |
