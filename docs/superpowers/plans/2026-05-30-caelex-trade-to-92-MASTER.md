...
| D — Schema & money          | 🟡 T-H7/M8/M9 DONE · T-H12 runbook-deferred | fix/trade-to-92: 7732778b·c679278a·fcc559d8                            | T-H7✅ atomic SAG draw-down (conditional updateMany increment + cap-bound WHERE; old absolute write GONE → no BAFA-cap overdraw; pure logic, 31/31 tests) · T-M8✅ @@unique([reportId,operationId]) on TradeSupplement2ReportItem · T-M9✅ TradeApiKey.createdBy onDelete SetNull + nullable (+ TradeApiKeyView type ripple fixed). Both additive → auto-applied by db-push. prisma validate ✅. **T-H12 (Float→BigInt money) DEFERRED — `docs/TRADE-MONEY-MIGRATION-RUNBOOK-T-H12.md`**: vercel build:deploy runs `db push --accept-data-loss` which DROPS+recreates a column on type change → would zero prod money; needs a human-run 2-phase Neon migration (USING value\*100 first, then schema). User constraint was 'nothing manual' so it's documented, not executed. Schema/Data-model →~88 (T-H12 closes the rest).                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
