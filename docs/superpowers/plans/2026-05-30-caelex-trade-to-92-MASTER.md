...
| B — Authz & API gate        | 🟡 B1+B2 done  | see `git log` feat(trade)     | B1✅ getTradeAuth() helper. B2✅ **T-H1** product-gate rolled into ALL 28 routes (42 handlers, 3 batches, IDOR guards intact, typecheck-clean, 349 screening green) + 8 route-gate tests on AI-cost routes (null-auth→403, AI client never called). NEXT: B3 Astra trade-tool gating (**T-H10**: mutating tools bypass approval + tool surface not product-gated). Remaining 26 route tests → Sprint I. |
