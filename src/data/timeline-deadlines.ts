...
export const autoGeneratedDeadlines: Record<ModuleType, DeadlineTemplate[]> = {
  ...
  DOCUMENTS: [
    {
      ...
      title: "Document Expiry Review",
      description: "Review and renew expiring compliance documents",
      offsetDays: -30,
      category: "INTERNAL",
      priority: "MEDIUM",
      regulatoryRef: "EU Space Act compliance documentation",
      reminderDays: [30, 14, 7],
    },
    {
      trigger: "document_uploaded",
      title: "Annual Document Audit",
      description: "Conduct annual audit of all compliance documentation",
      recurrence: "FREQ=YEARLY;BYMONTH=1;BYMONTHDAY=31",
      category: "INTERNAL",
      priority: "MEDIUM",
    },
  ],
  ...
  NIS2: [
    {
      trigger: "nis2_assessment_created",
      title: "NIS2 Entity Registration",
      description:
        "Register with the national competent authority as an essential or important entity under NIS2 Art. 3(4) and Art. 27",
      offsetDays: 90,
      category: "LICENSE" as DeadlineCategory,
      priority: "HIGH" as Priority,
      regulatoryRef: "NIS2 Art. 27",
      penaltyInfo:
        "Failure to register may result in administrative fines up to €10M or 2% of global annual turnover",
      reminderDays: [90, 60, 30, 14],
    },
    {
      trigger: "nis2_assessment_created",
      title: "NIS2 Art. 21 Measures Implementation",
      description:
        "Implement appropriate and proportionate cybersecurity risk-management measures under Art. 21",
      offsetDays: 180,
      category: "COMPLIANCE" as DeadlineCategory,
      priority: "CRITICAL" as Priority,
      regulatoryRef: "NIS2 Art. 21",
      penaltyInfo:
        "Non-compliance with Art. 21 measures may result in maximum fines of €10M or 2% of global annual turnover for essential entities",
      reminderDays: [180, 120, 90, 60, 30],
    },
    {
      trigger: "nis2_assessment_created",
      title: "Incident Reporting Capability",
      description:
        "Establish incident reporting capability to meet NIS2 Art. 23 timelines (24h early warning, 72h notification)",
      offsetDays: 60,
      category: "OPERATIONAL" as DeadlineCategory,
      priority: "CRITICAL" as Priority,
      regulatoryRef: "NIS2 Art. 23",
...
export const regulatoryDeadlines: RegulatoryDeadline[] = [
  ...
  },
  {
    ...
    offsetBeforeLaunch: 365, // 365 days BEFORE launch (positive = before)
    category: "REGULATORY",
    priority: "CRITICAL",
  ...
  },
  {
    ...
    offsetAfterLaunch: -365, // NOTE: Represents 365 days before EOL, not after launch. Needs schema refactor.
    category: "REGULATORY",
    priority: "CRITICAL",
    regulatoryRef: "EU Space Act Art. 28",
    applicableTo: ["SCO"],
...
// ============================================
// AUTO-GENERATE DEADLINES FROM TEMPLATES
// ============================================

/**
 * Map template category values to valid DeadlineCategory enum values.
 * Some templates (e.g. NIS2) use categories like COMPLIANCE/OPERATIONAL
 * that don't exist in the Prisma enum — remap them to valid values.
 */
const CATEGORY_REMAP: Record<string, DeadlineCategory> = {
  COMPLIANCE: "REGULATORY",
  OPERATIONAL: "MISSION",
};

function resolveCategory(cat: string): DeadlineCategory {
  if (cat in CATEGORY_REMAP) return CATEGORY_REMAP[cat];
  return cat as DeadlineCategory;
}

/**
 * Generate concrete deadline records from auto-generated templates.
 * Called after assessment creation to materialise regulatory deadlines
 * in the Deadline table.
 */
export function generateDeadlinesFromTemplate(
  ...
  const templates =
    autoGeneratedDeadlines[moduleType as keyof typeof autoGeneratedDeadlines];
  if (!templates || templates.length === 0) return [];

  return templates.map((template) => {
    const dueDate = new Date(
      triggerDate.getTime() + (template.offsetDays || 0) * 86_400_000,
    );
    return {
      ...
      penaltyInfo: template.penaltyInfo,
      moduleSource: moduleType as ModuleType,
      userId,
      organizationId,
      ...(template.reminderDays ? { reminderDays: template.reminderDays } : {}),
    };
  });
}
