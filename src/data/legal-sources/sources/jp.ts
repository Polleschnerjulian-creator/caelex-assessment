// src/data/legal-sources/sources/jp.ts

/**
 * Copyright 2026 Julian Polleschner (Caelex Einzelunternehmen). All rights reserved.
 *
 * Japan — space-law sources and authorities.
 *
 * Coverage: stub-level entry covering the core post-2008 framework (Basic
 * Space Law, 2016 Space Activities Act, 2021 Space Resources Act). Suffices
 * for cross-border due diligence between EU/UK/US operators and Japanese
 * counterparties; deeper coverage will follow when a Japan-licensee mandate
 * triggers the need.
 *
 * SPDX-License-Identifier: LicenseRef-Caelex-Proprietary
 */

import type { LegalSource, Authority } from "../types";

export const AUTHORITIES_JP: Authority[] = [
  {
    id: "JP-CABINET-OFFICE",
    jurisdiction: "JP",
    name_en: "Cabinet Office — Office of National Space Policy",
    name_local: "内閣府 宇宙開発戦略推進事務局",
    abbreviation: "Cabinet Office NSPO",
    parent_ministry: "Cabinet Office",
    website: "https://www8.cao.go.jp/space/",
    space_mandate:
      "Lead authority for licensing under the 2016 Space Activities Act. Issues launch and satellite-management permits, oversees the Space Resources Act registry, and operates the inter-ministerial Strategic Headquarters for Space Policy.",
    legal_basis: "Basic Space Law (2008); Cabinet Office Act",
    applicable_areas: [
      "licensing",
      "registration",
      "liability",
      "insurance",
      "debris_mitigation",
    ],
  },
  {
    id: "JP-METI",
    jurisdiction: "JP",
    name_en: "Ministry of Economy, Trade and Industry",
    name_local: "経済産業省",
    abbreviation: "METI",
    website: "https://www.meti.go.jp/english/policy/mono_info_service/space/",
    space_mandate:
      "Industrial policy for the space sector and dual-use export licensing under the Foreign Exchange and Foreign Trade Act. Co-administers the Space Resources Act with the Cabinet Office.",
    legal_basis: "Foreign Exchange and Foreign Trade Act (1949)",
    applicable_areas: ["export_control", "licensing"],
  },
  {
    id: "JP-MIC",
    jurisdiction: "JP",
    name_en: "Ministry of Internal Affairs and Communications",
    name_local: "総務省",
    abbreviation: "MIC",
    website: "https://www.soumu.go.jp/main_sosiki/joho_tsusin/eng/",
    space_mandate:
      "Frequency-spectrum allocation, satellite-radio-station licensing under the Radio Act, and ITU coordination filings on behalf of Japanese operators.",
    legal_basis: "Radio Act (1950)",
    applicable_areas: ["frequency_spectrum"],
  },
  {
    id: "JP-JAXA",
    jurisdiction: "JP",
    name_en: "Japan Aerospace Exploration Agency",
    name_local: "宇宙航空研究開発機構",
    abbreviation: "JAXA",
    website: "https://www.jaxa.jp/",
    space_mandate:
      "National space agency executing exploration, science, and the H3 / Epsilon launch capability. Acts as technical advisor to the Cabinet Office on Space Activities Act review.",
    legal_basis: "Independent Administrative Agency Act (2003)",
    applicable_areas: ["licensing", "debris_mitigation"],
  },
];

export const LEGAL_SOURCES_JP: LegalSource[] = [
  {
    id: "JP-BASIC-SPACE-LAW-2008",
    jurisdiction: "JP",
    type: "federal_law",
    status: "in_force",
    title_en: "Basic Space Law",
    title_local: "宇宙基本法",
    date_enacted: "2008-05-28",
    official_reference: "Act No. 43 of 2008",
    source_url: "https://www.japaneselawtranslation.go.jp/en/laws/view/2674",
    issuing_body: "Diet of Japan",
    competent_authorities: ["JP-CABINET-OFFICE"],
    relevance_level: "fundamental",
    applicable_to: ["all"],
    compliance_areas: ["licensing", "registration"],
    scope_description:
      "Constitutional anchor of the modern Japanese space-law framework. Establishes the Cabinet-led space-policy governance, opens the door to defence-oriented use of space (a 2008 break with the prior peaceful-purposes-only doctrine), and mandates the Government to enact a Space Activities Act — delivered in 2016.",
    key_provisions: [
      {
        section: "Art. 14",
        title: "Mandate to enact a Space Activities Act",
        summary:
          "Requires the Government to legislate the licensing, liability, insurance, and debris-mitigation regime for non-governmental space activities — fulfilled by the 2016 Space Activities Act.",
      },
      {
        section: "Art. 25-30",
        title: "Strategic Headquarters for Space Policy",
        summary:
          "Establishes the Cabinet-level Strategic Headquarters chaired by the Prime Minister, with the Office of National Space Policy as its secretariat.",
      },
    ],
    related_sources: ["JP-SPACE-ACTIVITIES-ACT-2016"],
    last_verified: "2026-04-22",
  },
  {
    id: "JP-SPACE-ACTIVITIES-ACT-2016",
    jurisdiction: "JP",
    type: "federal_law",
    status: "in_force",
    title_en: "Act on Launching of Spacecraft etc. and Control of Spacecraft",
    title_local: "人工衛星等の打上げ及び人工衛星の管理に関する法律",
    date_enacted: "2016-11-16",
    date_in_force: "2018-11-15",
    official_reference: "Act No. 76 of 2016",
    source_url: "https://www8.cao.go.jp/space/english/activities/launch.html",
    issuing_body: "Diet of Japan",
    competent_authorities: ["JP-CABINET-OFFICE"],
    relevance_level: "critical",
    applicable_to: [
      "launch_provider",
      "satellite_operator",
      "constellation_operator",
    ],
    compliance_areas: [
      "licensing",
      "liability",
      "insurance",
      "debris_mitigation",
      "registration",
    ],
    scope_description:
      "Operative licensing statute for Japanese commercial launch and on-orbit-management activities. Establishes a Cabinet-Office permit regime, mandatory third-party-liability insurance (currently ¥20 billion), Government indemnification above the insurance ceiling, and explicit debris-mitigation and end-of-life-disposal duties for satellite operators.",
    key_provisions: [
      {
        section: "Art. 4-13",
        title: "Launch permit regime",
        summary:
          "Launch operators require a Cabinet-Office permit covering vehicle qualification, flight-safety analysis, third-party-liability insurance, and pre-launch range coordination. Issued for a single launch or for series of launches of the same type.",
      },
      {
        section: "Art. 20-23",
        title: "Spacecraft management permit",
        summary:
          "Separate permit regime for on-orbit operation and disposal of artificial satellites, with explicit debris-mitigation, conjunction-coordination, and de-orbit-plan requirements.",
      },
      {
        section: "Art. 35-43",
        title: "Liability and Government indemnification",
        summary:
          "Operator strict liability for surface damage; ¥20 billion mandatory insurance; Government indemnification above the insurance cap up to a Diet-set ceiling.",
      },
    ],
    related_sources: ["JP-BASIC-SPACE-LAW-2008", "JP-SPACE-RESOURCES-2021"],
    last_verified: "2026-04-22",
  },
  {
    id: "JP-SPACE-RESOURCES-2021",
    jurisdiction: "JP",
    type: "federal_law",
    status: "in_force",
    title_en:
      "Act on Promotion of Business Activities for the Exploration and Development of Space Resources",
    title_local: "宇宙資源の探査及び開発に関する事業活動の促進に関する法律",
    date_enacted: "2021-06-15",
    date_in_force: "2021-12-23",
    official_reference: "Act No. 83 of 2021",
    source_url: "https://www8.cao.go.jp/space/policy/space_resources.html",
    issuing_body: "Diet of Japan",
    competent_authorities: ["JP-CABINET-OFFICE", "JP-METI"],
    relevance_level: "high",
    applicable_to: ["space_resource_operator"],
    compliance_areas: ["licensing"],
    scope_description:
      "Confers property rights on space resources extracted by Japanese-licensed operators, mirroring the US CSLCA Title IV approach. Japan is the fourth jurisdiction (after US, Luxembourg, UAE) to legislate space-resource ownership.",
    key_provisions: [
      {
        section: "Art. 5",
        title: "Recognition of property in extracted resources",
        summary:
          "An operator that has obtained a Cabinet-Office permit and successfully extracted space resources acquires ownership of those resources, consistent with Japan's interpretation of OST Art. II.",
      },
    ],
    related_sources: ["INT-OST-1967", "INT-ARTEMIS-ACCORDS-2020"],
    last_verified: "2026-04-22",
  },
  {
    id: "JP-FEFTA",
    jurisdiction: "JP",
    type: "federal_law",
    status: "in_force",
    title_en: "Foreign Exchange and Foreign Trade Act",
    title_local: "外国為替及び外国貿易法",
    date_enacted: "1949-12-01",
    date_last_amended: "2024-04-01",
    official_reference: "Act No. 228 of 1949 (as amended)",
    source_url: "https://www.japaneselawtranslation.go.jp/en/laws/view/4382",
    issuing_body: "Diet of Japan",
    competent_authorities: ["JP-METI"],
    relevance_level: "high",
    applicable_to: ["all"],
    compliance_areas: ["export_control"],
    scope_description:
      "Japan's primary export-control statute. Captures spacecraft and ground-segment items via the Export Trade Control Order (Cabinet Order 378/1949) and the Foreign Exchange Order (Cabinet Order 260/1980), with detailed item-listing in the METI Public Notice. Covers tangible exports, intangible technology transfers, and brokering.",
    key_provisions: [
      {
        section: "Art. 25 / 48",
        title: "Permit for controlled goods and technology",
        summary:
          "Export of items on the Strategic Goods List or transfer of associated technology requires a METI permit. Catch-all provisions capture unlisted items destined for WMD or military end-use.",
      },
      {
        section: "Art. 53",
        title: "End-user assurance and audit",
        summary:
          "Permit holders must maintain end-user records and submit to METI on-site inspection on demand. Penalties for permit violations include criminal liability and export-permit suspensions.",
      },
    ],
    related_sources: ["JP-SPACE-ACTIVITIES-ACT-2016", "INT-CN-EXPORT-LAW-2020"],
    last_verified: "2026-04-22",
  },
  {
    id: "JP-APPI",
    jurisdiction: "JP",
    type: "federal_law",
    status: "in_force",
    title_en: "Act on the Protection of Personal Information",
    title_local: "個人情報の保護に関する法律",
    date_enacted: "2003-05-30",
    date_last_amended: "2024-04-01",
    official_reference: "Act No. 57 of 2003 (as amended 2020, 2022, 2024)",
    source_url: "https://www.japaneselawtranslation.go.jp/en/laws/view/4241",
    issuing_body: "Diet of Japan",
    competent_authorities: [],
    relevance_level: "high",
    applicable_to: ["satellite_operator", "data_provider", "ground_segment"],
    compliance_areas: ["data_security"],
    scope_description:
      "Japan's general data-protection statute, often referred to as Japan's GDPR. Captures Earth-observation operators whose imagery resolves identifiable individuals or vehicles, satcom-subscriber data, and any cross-border transfer touching Japanese residents. Personal Information Protection Commission (PPC) is the enforcement authority.",
    key_provisions: [
      {
        section: "Art. 28",
        title: "Cross-border transfer restrictions",
        summary:
          "Transfers of personal data to recipients in third countries require either explicit consent, a binding-corporate-rules-equivalent mechanism, or transfer to a country recognised by PPC as providing adequate protection (currently UK, EU, etc.).",
      },
      {
        section: "Art. 26-27",
        title: "Breach reporting",
        summary:
          "Significant data breaches must be reported to PPC and to affected individuals; thresholds are lower than EU GDPR but enforcement is increasing.",
      },
    ],
    related_sources: ["EU-GDPR-2016"],
    last_verified: "2026-04-22",
  },
  {
    id: "JP-CYBERSECURITY-BASIC-ACT",
    jurisdiction: "JP",
    type: "federal_law",
    status: "in_force",
    title_en: "Cybersecurity Basic Act",
    title_local: "サイバーセキュリティ基本法",
    date_enacted: "2014-11-12",
    date_last_amended: "2024-06-01",
    official_reference: "Act No. 104 of 2014",
    source_url: "https://www.japaneselawtranslation.go.jp/en/laws/view/3677",
    issuing_body: "Diet of Japan",
    competent_authorities: [],
    relevance_level: "medium",
    applicable_to: ["satellite_operator", "ground_segment"],
    compliance_areas: ["cybersecurity"],
    scope_description:
      "Establishes the Cabinet Cybersecurity Strategic Headquarters and the National Center of Incident Readiness and Strategy for Cybersecurity (NISC). Designates space as a critical infrastructure sector — operators of designated systems face NISC-supervised risk-management duties broadly aligned with NIS2's structure.",
    key_provisions: [
      {
        section: "Art. 11-13",
        title: "Critical infrastructure framework",
        summary:
          "Sector-specific cybersecurity guidelines apply to designated critical-infrastructure operators. Space-sector designations have expanded since 2022 to include satellite ground-segments and TT&C facilities.",
      },
    ],
    related_sources: ["EU-NIS2-2022", "DE-BSIG-NIS2"],
    last_verified: "2026-04-22",
  },
  {
    id: "JP-ISS-AGREEMENT-IMPL",
    jurisdiction: "JP",
    type: "federal_law",
    status: "in_force",
    title_en: "Act on the Promotion of International Civil Use of Outer Space",
    title_local: "宇宙の平和的利用の促進等に関する法律",
    date_enacted: "1998-11-04",
    official_reference: "Act No. 124 of 1998",
    source_url: "https://www.japaneselawtranslation.go.jp/en/laws/view/2645",
    issuing_body: "Diet of Japan",
    competent_authorities: ["JP-CABINET-OFFICE", "JP-JAXA"],
    relevance_level: "medium",
    applicable_to: ["all"],
    compliance_areas: ["liability", "licensing"],
    scope_description:
      "Domestic implementing legislation for Japan's participation in the ISS Intergovernmental Agreement (INT-ISS-1998). Confers Japanese jurisdiction over the Kibo module and Japanese astronauts, and recognises the partner cross-waiver of liability that immunises ISS operators from claims arising from ISS activities.",
    key_provisions: [
      {
        section: "Art. 3-5",
        title: "Kibo jurisdiction and Japanese flight elements",
        summary:
          "Japan exercises jurisdiction and control over the Kibo Japanese Experiment Module and Japanese flight elements; the Cabinet Office coordinates Japanese partner-government activities under the IGA.",
      },
    ],
    related_sources: ["INT-ISS-1998"],
    last_verified: "2026-04-22",
  },
  {
    id: "JP-NEW-SPACE-VISION-2024",
    jurisdiction: "JP",
    type: "policy_document",
    status: "in_force",
    title_en: "Japan Space Industrial Vision 2024",
    title_local: "宇宙産業ビジョン2024",
    date_published: "2024-06-21",
    source_url:
      "https://www8.cao.go.jp/space/comittee/27-anpo/27-anpo-dai14/siryou1.pdf",
    issuing_body: "Cabinet Office / Strategic Headquarters for Space Policy",
    competent_authorities: ["JP-CABINET-OFFICE", "JP-METI"],
    relevance_level: "medium",
    applicable_to: ["all"],
    compliance_areas: ["licensing"],
    scope_description:
      "Cabinet-level industrial-policy strategy aimed at expanding Japan's space economy from ~¥4 trillion to ¥8 trillion by 2030. Identifies priority areas (launch competitiveness, satellite manufacturing, downstream services, space-resource utilisation) and aligns Government procurement levers with industrial outcomes. Non-binding policy but the funding context for METI/JAXA contracts.",
    key_provisions: [
      {
        section: "Pillar 1 — Launch capability",
        title: "Domestic launch resilience",
        summary:
          "Targets sustained H3 production cadence of 6+ launches/year by 2030 plus competitive small-launch capability via Space One and IST. Includes Government procurement guarantees.",
      },
    ],
    related_sources: [
      "JP-BASIC-SPACE-LAW-2008",
      "JP-SPACE-ACTIVITIES-ACT-2016",
    ],
    last_verified: "2026-04-22",
  },
  {
    id: "JP-RADIO-ACT",
    jurisdiction: "JP",
    type: "federal_law",
    status: "in_force",
    title_en: "Radio Act (Japan)",
    title_local: "電波法",
    date_enacted: "1950-05-02",
    official_reference: "Act No. 131 of 1950 (as amended)",
    source_url: "https://www.japaneselawtranslation.go.jp/en/laws/view/4124",
    issuing_body: "Diet of Japan",
    competent_authorities: ["JP-MIC"],
    relevance_level: "high",
    applicable_to: [
      "satellite_operator",
      "constellation_operator",
      "ground_segment",
    ],
    compliance_areas: ["frequency_spectrum"],
    scope_description:
      "Frequency-licensing statute administered by MIC for every radio-equipped satellite system with a Japanese gateway, operator, or end-user. Companion to the Space Activities Act on the spectrum side; ITU coordination flows through MIC notifications.",
    key_provisions: [
      {
        section: "Art. 4",
        title: "Radio-station licence required",
        summary:
          "All radio stations including satellite TT&C and payload links require an MIC licence. Foreign operators serving Japanese end-users typically require a Japanese earth-station partner.",
      },
    ],
    related_sources: ["JP-SPACE-ACTIVITIES-ACT-2016"],
    last_verified: "2026-04-22",
  },

  // ─── Debris-Mitigation national stack — 2026 audit additions ───────

  {
    id: "JP-JAXA-SED-STD",
    jurisdiction: "JP",
    type: "technical_standard",
    status: "in_force",
    title_en: "JAXA Space Debris Mitigation Standard (JMR-003)",
    title_local: "宇宙航空研究開発機構 デブリ発生防止標準 JMR-003",
    date_published: "2014-11-30",
    date_last_amended: "2023-04-01",
    official_reference: "JMR-003D (Rev. D, 2023)",
    source_url: "https://sma.jaxa.jp/en/TechDoc/Docs/JAXA-JMR-003D_E.pdf",
    issuing_body: "Japan Aerospace Exploration Agency (JAXA)",
    competent_authorities: ["JP-JAXA"],
    relevance_level: "fundamental",
    applicable_to: ["satellite_operator", "launch_provider"],
    compliance_areas: ["debris_mitigation"],
    scope_description:
      "JAXA's binding internal debris-mitigation standard, applied to all JAXA missions and to commercial spacecraft launched on JAXA-procured launchers. Implements the IADC guidelines with a Japan-specific 1×10⁻⁶ casualty-risk target — the world's strictest re-entry casualty threshold — driven by the high population density along potential disposal ground-tracks. Rev. D (April 2023) added a 5-year LEO PMD preference consistent with IADC 2025.",
    key_provisions: [
      {
        section: "§5.4",
        title: "Casualty risk — 1:1,000,000",
        summary:
          "Aggregate human-casualty risk for any uncontrolled re-entry shall not exceed 1×10⁻⁶ — ten times stricter than the international 10⁻⁴ baseline.",
        complianceImplication:
          "Effectively forces controlled re-entry for all spacecraft with surviving fragments > 0.1 kg. Cited by JAXA as the reason for design choices (e.g. fully-burning materials, pre-emptive demisability analysis) on missions like ETS-9 and ALOS-4.",
      },
      {
        section: "§5.6",
        title: "PMD reliability ≥ 0.95",
        summary:
          "Probability of successful post-mission disposal ≥ 0.95; redundant disposal mechanisms required for missions exceeding 100 kg.",
      },
    ],
    related_sources: [
      "JP-SPACE-ACTIVITIES-ACT-2016",
      "INT-IADC-MITIGATION-2025",
      "INT-ISO-24113-2023",
    ],
    last_verified: "2026-04-27",
  },

  {
    id: "JP-CABINET-DEBRIS-GUIDELINE-2024",
    jurisdiction: "JP",
    type: "policy_document",
    status: "in_force",
    title_en:
      "Cabinet Office Guideline on Space Debris Mitigation for Commercial Operators",
    title_local: "民間事業者向け 宇宙デブリ発生防止に関するガイドライン",
    date_published: "2024-08-15",
    official_reference: "Cabinet Office Notice No. 2024-08",
    source_url: "https://www8.cao.go.jp/space/policy/debris/",
    issuing_body: "Cabinet Office of Japan — Office of National Space Policy",
    competent_authorities: ["JP-CABINET-OFFICE"],
    relevance_level: "high",
    applicable_to: ["satellite_operator", "launch_provider"],
    compliance_areas: ["debris_mitigation", "licensing"],
    scope_description:
      "Cabinet Office guideline binding on commercial operators applying for licences under the 2016 Space Activities Act. References JMR-003 as the technical standard and aligns the licence-condition language with FCC 2022 / IADC 2025: 5-year LEO PMD, 0.95 disposal-reliability, and active conjunction-data-sharing with JAXA Space Situational Awareness Centre.",
    key_provisions: [
      {
        section: "§3",
        title: "Licence conditions",
        summary:
          "Commercial operators must demonstrate JMR-003 compliance in the technical submission under the Space Activities Act licence application.",
      },
      {
        section: "§5",
        title: "Conjunction-data sharing",
        summary:
          "Operators of Japan-licensed spacecraft must share manoeuvre plans and ephemerides with JAXA SSA Centre via CCSDS-formatted exchange.",
      },
    ],
    related_sources: [
      "JP-SPACE-ACTIVITIES-ACT-2016",
      "JP-JAXA-SED-STD",
      "INT-CCSDS-CDM-508",
    ],
    last_verified: "2026-04-27",
  },

  // ═══════════════════════════════════════════════════════════════════════
  // Atlas P2 (2026-05-26): JP sub-domain entries — CRD2 ADR demonstration.
  // ═══════════════════════════════════════════════════════════════════════
  {
    id: "JP-CRD2-MISSION-FRAMEWORK",
    jurisdiction: "JP",
    type: "procurement_framework",
    status: "in_force",
    title_en: "JAXA Commercial Removal of Debris Demonstration (CRD2)",
    date_published: "2020-02-13",
    source_url: "https://www.kenkai.jaxa.jp/eng/business/crd2.html",
    issuing_body: "Japan Aerospace Exploration Agency",
    competent_authorities: ["JP-JAXA"],
    relevance_level: "high",
    applicable_to: ["in_orbit_services"],
    compliance_areas: ["debris_mitigation", "procurement"],
    scope_description:
      "JAXA's flagship commercial debris-removal demonstration programme. Phase I (2020-2024) awarded to Astroscale Japan — ADRAS-J mission inspected H-IIA upper stage 2024. Phase II (2024 onwards) targets actual deorbit demonstration. Contractual architecture: JAXA-as-customer purchasing a debris-removal service, not operating the mission. Material precedent for Asia-Pacific ADR market.",
    key_provisions: [
      {
        section: "Phase I + Phase II structure",
        title: "Service-acquisition model",
        summary:
          "Phase I: rendezvous + proximity operations + characterisation. Phase II: capture + controlled deorbit. Service-acquisition model parallel to ESA ClearSpace-1.",
      },
    ],
    related_sources: [
      "EU-CLEARSPACE-1-FRAMEWORK",
      "INT-IADC-ADR-STATEMENT-2023",
    ],
    last_verified: "2026-05-26",
  },

  // ═══════════════════════════════════════════════════════════════════════
  // Atlas P4 (2026-05-26): Japan sub-tier deepening — Defense Space
  // Strategy 2020, National Security Strategy 2022, MTCR founding member,
  // Artemis Accords founding cohort, Hayabusa + MMX sample-return, QZSS
  // regional augmentation, H3 commercial framework, IST first commercial
  // orbital, 4th Basic Space Plan. Per ATLAS-CORPUS-EXPANSION-PLAN.md
  // § 8.D.
  // ═══════════════════════════════════════════════════════════════════════

  // ─── Defence + National Security ─────────────────────────────────────
  {
    id: "JP-DEFENSE-SPACE-STRATEGY-2020",
    jurisdiction: "JP",
    type: "national_security_doctrine",
    status: "in_force",
    title_en: "Japan Defense Space Strategy",
    title_local: "防衛省宇宙基本方針",
    date_published: "2020-06-30",
    source_url: "https://www.mod.go.jp/en/",
    issuing_body: "Ministry of Defense",
    competent_authorities: ["JP-CABINET-OFFICE"],
    relevance_level: "high",
    applicable_to: ["all"],
    compliance_areas: ["military_dual_use", "fdi_screening"],
    scope_description:
      "Foundational defence-space strategy. Established Space Operations Squadron (Air Self-Defense Force) 2020 — Japan's first dedicated military-space unit. 2024 reorganisation expanded to Space Operations Group with deep-space radar + electronic-warfare capabilities. Material for FDI screening of Japanese space-tech firms with defence touchpoints.",
    key_provisions: [],
    related_sources: ["JP-NATIONAL-SECURITY-STRATEGY-2022"],
    last_verified: "2026-05-26",
  },
  {
    id: "JP-NATIONAL-SECURITY-STRATEGY-2022",
    jurisdiction: "JP",
    type: "national_security_doctrine",
    status: "in_force",
    title_en: "Japan National Security Strategy 2022",
    title_local: "国家安全保障戦略 2022",
    date_published: "2022-12-16",
    source_url: "https://www.cas.go.jp/jp/siryou/221216anzenhoshou.html",
    issuing_body: "Cabinet Office",
    competent_authorities: ["JP-CABINET-OFFICE"],
    relevance_level: "high",
    applicable_to: ["all"],
    compliance_areas: ["military_dual_use", "fdi_screening"],
    scope_description:
      "First major NSS update since 2013. Includes substantial space-security provisions: dual-use space technology promotion, resilient PNT (QZSS expansion), counter-space deterrent capability development. Triggered Defense Build-up Plan 2023 with ¥1.7 trillion+ space-related spending through 2027. Material context for foreign space-tech firms considering Japan defence-procurement participation.",
    key_provisions: [],
    related_sources: ["JP-DEFENSE-SPACE-STRATEGY-2020"],
    last_verified: "2026-05-26",
  },

  // ─── International cooperation ────────────────────────────────────────
  {
    id: "JP-ARTEMIS-ACCORDS-2020",
    jurisdiction: "JP",
    type: "bilateral_agreement",
    status: "in_force",
    title_en: "Japan Artemis Accords Signing",
    date_enacted: "2020-10-13",
    source_url: "https://www.nasa.gov/news-release/artemis-accords/",
    issuing_body: "Government of Japan",
    competent_authorities: ["JP-CABINET-OFFICE"],
    relevance_level: "medium",
    applicable_to: ["all"],
    compliance_areas: ["procurement"],
    scope_description:
      "Japan founding-cohort Artemis Accords signatory (October 2020). Followed by deepening US-Japan space cooperation: JAXA-NASA Gateway lunar-station partnership (Japan provides HTV-X lunar logistics), Astronaut Onishi to Artemis IV (planned 2027), JAXA pressurised lunar rover (LUPEX successor). Material foundation for Japanese commercial-lunar service exports to US prime contractors.",
    key_provisions: [],
    related_sources: ["INT-ARTEMIS-ACCORDS-2020"],
    last_verified: "2026-05-26",
  },
  {
    id: "JP-MTCR-MEMBERSHIP-1987",
    jurisdiction: "JP",
    type: "multilateral_agreement",
    status: "in_force",
    title_en: "Japan MTCR Founding Membership",
    date_enacted: "1987-04-16",
    source_url: "https://mtcr.info/",
    issuing_body: "Missile Technology Control Regime",
    competent_authorities: ["JP-METI"],
    relevance_level: "medium",
    applicable_to: ["launch_provider"],
    compliance_areas: ["export_control"],
    scope_description:
      "Japan is founding member of MTCR (1987). Material constraint on H-IIA/H-IIB/H3/Epsilon export — controlled technology + non-proliferation requirements baked into all Japanese launch-vehicle technology transfer. METI's Strategic Goods List aligned with MTCR Annex.",
    key_provisions: [],
    related_sources: ["JP-FEFTA"],
    last_verified: "2026-05-26",
  },

  // ─── QZSS regional navigation ────────────────────────────────────────
  {
    id: "JP-QZSS-FRAMEWORK",
    jurisdiction: "JP",
    type: "federal_regulation",
    status: "in_force",
    title_en: "Quasi-Zenith Satellite System (QZSS) Operational Framework",
    title_local: "準天頂衛星システム — 運用枠組み",
    date_enacted: "2010-09-11",
    date_last_amended: "2024-01-01",
    source_url: "https://qzss.go.jp/en/",
    issuing_body: "Cabinet Office / National Space Policy Secretariat",
    competent_authorities: ["JP-CABINET-OFFICE"],
    relevance_level: "high",
    applicable_to: ["satellite_operator", "data_provider"],
    compliance_areas: ["frequency_spectrum"],
    scope_description:
      "QZSS — Japan's regional satellite navigation system, GPS-augmentation for Japan + East Asia + Australia. 7 satellites planned by 2026 (currently 4 operational). L1/L5 compatibility with GPS established under US-Japan agreement 2002+. Sub-Centimeter Augmentation Service (CLAS) provides commercial high-precision positioning.",
    key_provisions: [],
    related_sources: ["INT-GPS-QZSS-COMPATIBILITY"],
    last_verified: "2026-05-26",
  },

  // ─── Sample-return + deep-space ──────────────────────────────────────
  {
    id: "JP-HAYABUSA-FRAMEWORK",
    jurisdiction: "JP",
    type: "policy_document",
    status: "in_force",
    title_en: "Hayabusa Sample-Return Mission Framework",
    title_local: "はやぶさサンプルリターン",
    date_published: "2003-05-09",
    date_last_amended: "2020-12-06",
    source_url: "https://www.hayabusa.jaxa.jp/",
    issuing_body: "Japan Aerospace Exploration Agency",
    competent_authorities: ["JP-JAXA"],
    relevance_level: "medium",
    applicable_to: ["all"],
    compliance_areas: ["scientific_research", "environmental"],
    scope_description:
      "Hayabusa (2003-2010) + Hayabusa-2 (2014-2020) asteroid sample-return mission framework. First-ever sample returns from asteroids (Itokawa, Ryugu). COSPAR Planetary Protection Category V (restricted Earth-return) compliance. Material precedent for upcoming sample-return missions (Martian Moons Exploration MMX 2026, ESA Hera follow-on).",
    key_provisions: [],
    related_sources: ["INT-COSPAR-PLANETARY-PROTECTION"],
    last_verified: "2026-05-26",
  },
  {
    id: "JP-MMX-2026",
    jurisdiction: "JP",
    type: "policy_document",
    status: "in_force",
    title_en:
      "Martian Moons Exploration (MMX) — Phobos Sample-Return Framework",
    date_published: "2017-06-09",
    source_url: "https://mmx.isas.jaxa.jp/en/",
    issuing_body: "JAXA / ISAS / Cabinet Office",
    competent_authorities: ["JP-JAXA"],
    relevance_level: "low",
    applicable_to: ["all"],
    compliance_areas: ["scientific_research", "environmental"],
    scope_description:
      "JAXA's MMX mission targeting Phobos sample return (launch planned 2026). First-ever Mars-system sample return. CNES collaboration (rover IDEFIX). COSPAR Planetary Protection Category V (Mars-system Earth-return) — strictest planetary-protection regime. Material precedent for restricted-Earth-return commercial-mission protocols.",
    key_provisions: [],
    related_sources: [
      "JP-HAYABUSA-FRAMEWORK",
      "INT-COSPAR-PLANETARY-PROTECTION",
    ],
    last_verified: "2026-05-26",
  },

  // ─── JAXA Commercial H3 + IST ────────────────────────────────────────
  {
    id: "JP-H3-COMMERCIAL-SERVICES",
    jurisdiction: "JP",
    type: "procurement_framework",
    status: "in_force",
    title_en: "JAXA H3 Commercial Launch Services Framework",
    title_local: "H3ロケット商業打ち上げサービス",
    date_published: "2024-02-17",
    source_url: "https://global.jaxa.jp/projects/rockets/h3/",
    issuing_body: "JAXA + Mitsubishi Heavy Industries",
    competent_authorities: ["JP-JAXA"],
    relevance_level: "medium",
    applicable_to: ["launch_provider"],
    compliance_areas: ["procurement"],
    scope_description:
      "H3 commercial launch services framework. H3 first successful launch February 2024 after IFT-1 failure March 2023. MHI as prime contractor + commercial launch operator. Material framework for foreign customers seeking Japanese launch services (KASA's Korea Pathfinder Lunar Orbiter launched on Falcon 9, but future Korean lunar missions targeting H3).",
    key_provisions: [],
    related_sources: [],
    last_verified: "2026-05-26",
  },
  {
    id: "JP-IST-INTERSTELLAR-TECHNOLOGIES",
    jurisdiction: "JP",
    type: "policy_document",
    status: "in_force",
    title_en:
      "Interstellar Technologies (IST) — Japan's First Private Orbital Launch",
    date_published: "2023-08-15",
    source_url: "https://www.istellartech.com/en/",
    issuing_body: "Interstellar Technologies Inc.",
    competent_authorities: ["JP-CABINET-OFFICE"],
    relevance_level: "low",
    applicable_to: ["launch_provider"],
    compliance_areas: ["licensing"],
    scope_description:
      "IST — Japan's leading commercial launch startup. MOMO suborbital + ZERO orbital launcher development. Targeting Japan's first private orbital launch ~2025-2026. Operates Hokkaido Spaceport (Taiki-cho). Material precedent for Japanese commercial launch ecosystem alongside Honda + Toyota-backed efforts.",
    key_provisions: [],
    related_sources: ["JP-SPACE-ACTIVITIES-ACT-2016"],
    last_verified: "2026-05-26",
  },

  // ─── Commercial promotion + Basic Space Plan ─────────────────────────
  {
    id: "JP-CABINET-COMMERCIAL-PROMO-2024",
    jurisdiction: "JP",
    type: "policy_document",
    status: "in_force",
    title_en: "Japan Commercial Space Promotion Plan 2024",
    title_local: "宇宙ビジネス発展加速化計画 2024",
    date_published: "2024-06-13",
    source_url: "https://www8.cao.go.jp/space/policy/seisaku-anzen-hosho.html",
    issuing_body: "Cabinet Office National Space Policy Secretariat",
    competent_authorities: ["JP-CABINET-OFFICE"],
    relevance_level: "medium",
    applicable_to: ["all"],
    compliance_areas: ["state_aid", "procurement"],
    scope_description:
      "Cabinet-level commercial-space promotion plan. ¥10 trillion target Japanese space economy by 2030. Tax incentives for space-tech R&D + sovereign-wealth-fund participation. Coordinated with 4th Basic Space Plan 2023. Material for foreign-Japanese space JV structuring + investment.",
    key_provisions: [],
    related_sources: [],
    last_verified: "2026-05-26",
  },
  {
    id: "JP-BASIC-SPACE-PLAN-4TH-2023",
    jurisdiction: "JP",
    type: "policy_document",
    status: "in_force",
    title_en: "4th Basic Space Plan (2023-2032)",
    title_local: "第4期宇宙基本計画",
    date_published: "2023-06-13",
    source_url: "https://www8.cao.go.jp/space/plan/keikaku.html",
    issuing_body: "Cabinet Office Strategic Headquarters for Space Policy",
    competent_authorities: ["JP-CABINET-OFFICE"],
    relevance_level: "high",
    applicable_to: ["all"],
    compliance_areas: ["procurement", "state_aid"],
    scope_description:
      "Cabinet-approved 10-year strategic plan governing all Japanese space activities. Successor to 3rd Basic Space Plan 2015. Key priorities: H3 transition, lunar exploration (LUPEX + Gateway HTV-X), Mars Moons MMX, defence space build-up, commercial-services expansion (¥10T sector by 2030). Material strategic context for any Japan-related space planning.",
    key_provisions: [],
    related_sources: ["JP-CABINET-COMMERCIAL-PROMO-2024"],
    last_verified: "2026-05-26",
  },

  // ─── Economic Security / FDI ─────────────────────────────────────────
  {
    id: "JP-ECONOMIC-SECURITY-ACT-2022",
    jurisdiction: "JP",
    type: "federal_law",
    status: "in_force",
    title_en: "Economic Security Promotion Act",
    title_local: "経済安全保障推進法",
    date_enacted: "2022-05-11",
    date_in_force: "2023-04-01",
    official_reference: "Act No. 43 of 2022",
    source_url: "https://www.cao.go.jp/keizai_anzen_hosho/",
    issuing_body: "National Diet of Japan",
    competent_authorities: ["JP-CABINET-OFFICE"],
    relevance_level: "high",
    applicable_to: ["all"],
    compliance_areas: ["fdi_screening", "critical_infrastructure"],
    scope_description:
      "Foundational economic-security statute. Establishes (i) critical-infrastructure pre-approval (satcom + ground-segment included for designated entities), (ii) supply-chain resilience for critical materials (semiconductors used in space hardware), (iii) sensitive-technology development framework, (iv) patent non-disclosure for security-sensitive technologies. Material for any foreign space-tech firm with Japanese supply-chain or customer relationships.",
    key_provisions: [
      {
        section: "Pillar 1 - Critical Infrastructure",
        title: "Pre-approval for foreign-supplier procurement",
        summary:
          "Designated CII operators (incl. major satcom) must obtain government pre-approval before procuring specified equipment from foreign suppliers — material for Huawei/ZTE-equivalent screening.",
      },
    ],
    related_sources: ["JP-CYBERSECURITY-BASIC-ACT"],
    last_verified: "2026-05-26",
  },

  // ─── Astronaut programme ─────────────────────────────────────────────
  {
    id: "JP-ASTRONAUT-PROGRAMME-FRAMEWORK",
    jurisdiction: "JP",
    type: "federal_regulation",
    status: "in_force",
    title_en: "JAXA Astronaut Programme Framework",
    title_local: "JAXA宇宙飛行士プログラム",
    date_enacted: "2009-04-01",
    date_last_amended: "2023-02-28",
    source_url: "https://humans-in-space.jaxa.jp/en/",
    issuing_body: "JAXA",
    competent_authorities: ["JP-JAXA"],
    relevance_level: "medium",
    applicable_to: ["all"],
    compliance_areas: ["human_spaceflight", "employment_labor"],
    scope_description:
      "Operational framework for JAXA astronaut corps. 2023 selection cohort (Yui Akihiko + Suwa Makoto) — first new astronaut selection in 14 years. Material framework for JAXA contribution to Artemis programme (lunar surface mission slot confirmed) + ISS mission crew exchange agreements. Cross-references AX-3 commercial-astronaut framework.",
    key_provisions: [],
    related_sources: ["INT-ISS-IGA-1998", "JP-ISS-AGREEMENT-IMPL"],
    last_verified: "2026-05-26",
  },
];
