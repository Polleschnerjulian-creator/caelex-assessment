/**
 * Route-gate tests for GET /api/trade/customs/atlas-de (T-H1 Sprint I / I1).
 *
...
vi.mock("@/lib/trade/customs-filing/atlas-de", () => ({
  buildAtlasXml: vi.fn().mockReturnValue("<ATLAS/>"),
}));
...
function makeReq(operationId?: string): Request {
  const url = operationId
    ? `http://localhost/api/trade/customs/atlas-de?operationId=${operationId}`
    : "http://localhost/api/trade/customs/atlas-de";
...
describe("GET /api/trade/customs/atlas-de — auth gate (T-H1)", () => {
  beforeEach(() => {
    vi.resetModules();
  });
