/**
 * Route-gate tests for GET /api/trade/operations/[id]/sham-risk
...
// Stub the sham detector so it never runs on the gate path
vi.mock("@/lib/trade/ofac-sham-doctrine/detector", () => ({
  detectShamTransactionRisk: vi.fn().mockReturnValue({
    riskScore: 0,
    redFlags: [],
...
function makeReq(): Request {
  return new Request("http://localhost/api/trade/operations/op-1/sham-risk", {
    method: "GET",
  });
}
...
describe("GET /api/trade/operations/[id]/sham-risk — auth gate (T-H1)", () => {
  beforeEach(() => {
    vi.resetModules();
  });
