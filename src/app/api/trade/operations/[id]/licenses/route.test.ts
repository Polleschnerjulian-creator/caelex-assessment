/**
 * Route-gate tests for POST /api/trade/operations/[id]/licenses (T-H1 Sprint I / I1).
 *
...
function makeReq(body?: unknown): Request {
  return new Request("http://localhost/api/trade/operations/op-1/licenses", {
    method: "POST",
...
describe("POST /api/trade/operations/[id]/licenses — auth gate (T-H1)", () => {
  beforeEach(() => {
    vi.resetModules();
  });
  ...
  it("does NOT return 403 from the product gate when auth is valid (sanity — fails at validation instead)", async () => {
    const { getTradeAuth } = await import("@/lib/trade/trade-auth");
    vi.mocked(getTradeAuth).mockResolvedValue(validAuth);

    const { POST } = await import("./route");
    // Missing required licenseId field — Zod validation fires, not the auth gate
    const res = await POST(makeReq({ notALicenseId: "bad" }), ctx());
