...
function AuditCenterContent() {
  ...
  // Tab
  const [activeTab, setActiveTab] = useState<"evidence" | "attestations">(
    "evidence",
  );
  ...
  return (
    <div className="space-y-8">
      ...
      {/* ─── Tab Bar ─── */}
      <div className="flex items-center gap-1 p-1 bg-[var(--surface-sunken)] rounded-lg w-fit">
        {(
          [
          ...
          return (
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-md text-small font-medium transition-colors ${
                activeTab === tab.id
                  ? "bg-[var(--surface-sunken)] text-[var(--text-primary)] shadow-sm"
          ...
          >
            ...
            {filteredModules.length === 0 && (
              <div className="glass-card rounded-[var(--radius-lg)] p-8 text-center">
                ...
                <p className="text-[13px] text-[var(--text-tertiary)]">
                  No compliance modules found. Start an assessment to see data
                  here.
                </p>
              </div>
            )}
