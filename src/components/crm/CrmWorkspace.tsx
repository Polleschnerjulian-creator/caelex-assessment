...
function CrmPageContent() {
  ...
  const changeTab = useCallback(
    (newTab: Tab) => {
      ...
      router.replace(`/admin/crm?${params.toString()}`);
    },
    [searchParams, router],
  );
              ...
              {contacts.map((c) => {
                ...
                return (
                  <tr
                    ...
                    onClick={() =>
                      (window.location.href = `/admin/crm/contacts/${c.id}`)
                    }
                  >
            ...
            >
              {deals.map((d) => (
                <tr
                  ...
                  className="hover:bg-[var(--surface-sunken)] transition-colors cursor-pointer"
                  onClick={() =>
                    (window.location.href = `/admin/crm/deals/${d.id}`)
                  }
                >
                  <td className="px-4 py-3">
                    <p className="text-body font-medium text-[var(--text-primary)]">
