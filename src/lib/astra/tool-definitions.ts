...
export const evaluateShamRisk: AstraToolDefinition = {
  ...
  description:
    "Run the OFAC Sham-Transaction Doctrine detector (Z16, per 31 CFR § 501.601 and OFAC's 31 March 2026 Sanctions Advisory on Sham Transactions and Sanctions Evasion) against a Trade operation. Evaluates the six red-flag categories: indirect ownership chain (UBO depth > 3), shell-company markers (age < 12mo + 0 employees + shell-jurisdiction), geography mismatch, payment-routing divergence, pricing anomaly, and re-export risk history. Returns: aggregated 0-100 risk score, categorical recommendation (PROCEED / ENHANCED_DUE_DILIGENCE / ESCALATE / REJECT), red-flag list with severity + rationale + enforcement-action citations, and skipped-checks list (data gaps surfaced explicitly per the GVA Capital settlement). Use when the operator asks 'check sham risk on OP-XXX', 'why did Caelex flag this operation?', or 'is this transaction at risk of being recharacterised as a sham?'.",
  input_schema: {
    type: "object",
    properties: {
      operationId: {
