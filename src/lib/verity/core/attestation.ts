...
export function generateAttestation(
  ...
  // 2. Create commitment (binds Caelex to the real value)
  ...
  // 3. Trust score → external trust level
  const trust_info = toExternalTrust(params.trust_score);

  // 4. Build the attestation (WITHOUT actual_value, WITHOUT trust_score float)
  const attestation_unsigned = {
    attestation_id: `va_${Date.now()}_${randomBytes(8).toString("hex")}`,
