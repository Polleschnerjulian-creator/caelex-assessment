...
// Map activity types to operator keys and abbreviations
...
// Flatten all articles from the JSON structure
...
// Filter articles by operator type
function filterArticlesByOperator(
  ...
  return articles.filter((article) => {
    ...
    if (excludes.includes(operatorAbbrev)) {
      return false;
    }
    if (isThirdCountry && excludes.includes("TCO")) {
      return false;
    }

    // Check if applies to this operator
    if (appliesTo.includes("ALL")) {
      return true;
    }
    if (appliesTo.includes(operatorAbbrev)) {
      return true;
    }
    if (isThirdCountry && appliesTo.includes("TCO")) {
      return true;
    }
...
// Get article number as integer for comparison
function getArticleNumber(article: Article): number {
  const num = article.number;
  if (typeof num === "number") {
    return num;
  }
  // Handle ranges like "17-19" or "32-36"
  const match = String(num).match(/^(\d+)/);
  return match ? parseInt(match[1], 10) : 0;
}

// Parse an articleRange string like "Art. 6–16, 32–39, 105–108" into ranges
function parseArticleRange(
  articleRange: string,
): { start: number; end: number }[] {
  const ranges: { start: number; end: number }[] = [];
  // Remove "Art." prefix and split by comma
  const cleaned = articleRange.replace(/Art\.\s*/g, "");
  const parts = cleaned.split(",").map((p) => p.trim());

  for (const part of parts) {
    // Handle ranges with – (en-dash), - (hyphen), or single numbers
...
// Check if an article number falls within any of the ranges
...
function calculateModuleStatuses(
  ...
  return MODULES.map((module) => {
    // Parse the article range string into actual ranges
    const ranges = parseArticleRange(module.articleRange);

    // Filter articles that fall within this module's article range
    ...
    if (articleCount > 0) {
      ...
      // Check for simplified articles (light regime)
...
// Get checklist based on operator type
function getChecklist(
  ...
  if (isThirdCountry) {
    ...
    // Default to spacecraft operator checklist for other types
    const scoChecklist = checklists.spacecraft_operator_eu;
    checklist = [
      ...(scoChecklist.pre_authorization || []),
      ...scoChecklist.ongoing,
...
// Get authorization cost estimate
function getAuthorizationCost(
  operatorType: OperatorTypeKey,
  isThirdCountry: boolean,
): string {
  if (isThirdCountry) {
    return "Registration fee (TBD by EUSPA)";
  }
...
function getAuthorizationPath(isThirdCountry: boolean, isEU: boolean): string {
  if (isThirdCountry) {
    return "EUSPA Registration → Commission Decision";
  }
  if (isEU) {
    return "National Authority (NCA) → URSO Registration";
  }
...
export function calculateCompliance(
  ...
  // 2. Determine EU vs Third Country status
...
// Utility function to load and parse the JSON data
export async function loadSpaceActData(): Promise<SpaceActData> {
  const response = await fetch("/caelex-eu-space-act-engine.json");
  if (!response.ok) {
    throw new Error("Failed to load Space Act data");
  }
