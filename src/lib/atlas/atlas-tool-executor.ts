...
/* getFilingDeadlines + helpers (FilingDeadlinesInput,
   OPERATOR_CODE_MAP, inferDeadlineJurisdiction, nextAnnualOccurrence)
   moved to deadlines-tools.server.ts as part of Atlas V3 T0.1.g
   bundle-split (2026-05-26). Routed via the isDeadlinesToolName
...
export async function executeAtlasTool(args: {
  ...
  switch (args.name as AtlasToolName) {
    ...
    /* get_filing_deadlines early-routed via isDeadlinesToolName above
       (Atlas V3 T0.1.g bundle-split). */
